...
export default function Activities() {
  ...
  const { t, activities: activitiesData, updateData } = useLanguage()
  const { isCompact } = useCollapse()
  const { isEditMode, setHasChanges } = useEdit()

  // Convertir le texte en liste d'éléments
  const items = useMemo(() => {
    if (!activitiesData?.text) return []
    return activitiesData.text.split(' | ').filter(item => item.trim())
  }, [activitiesData?.text])
  ...
  const handleItemChange = (idx, newValue) => {
    ...
    updateData('activities.text', newItems.join(' | '))
    setHasChanges(true)
  }
