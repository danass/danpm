...
import pageDataJson from '@/public/data/case-study-structure.json'; // Renamed to avoid conflict
import Button from '../components/design-system/Button'; // Assuming Button component exists

export default function CaseStudyPage() {
  const searchParams = useSearchParams();
  const [isEditing, setIsEditing] = useState(false);
  // Initialize with a clean copy, ensuring no leftover _isPendingDeletion flags
  ...
  useEffect(() => {
    if (!isEditing) {
      // Reset to original data, ensuring _isPendingDeletion flags are cleared
  ...
  const handleBlockPropChange = useCallback((blockIndex, contentIndex, propPathArray, newValue) => {
    setEditableBlocks(prevBlocks => {
      const newBlocks = [...prevBlocks];
      const newBlock = JSON.parse(JSON.stringify(newBlocks[blockIndex])); // Deep clone the specific block to ensure immutability

      let currentTarget;
      if (contentIndex !== undefined && newBlock.props.content && newBlock.props.content[contentIndex]) {
        // Target is a component within CustomSection's content array
        currentTarget = newBlock.props.content[contentIndex].props;
      } else {
        // Target is the props of the top-level block component
      ...
      for (let i = 0; i < propPathArray.length - 1; i++) {
        ...
        if (tempTarget[key] === undefined || typeof tempTarget[key] !== 'object') {
          // If path doesn't exist or is not an object/array, create it.
          // This is a basic way; more robust handling might be needed for complex cases.
  ...
  const handleSaveChanges = async () => {
    const blocksToSave = editableBlocks
      ...
      .map(block => {
        // Clean up the _isPendingDeletion flag before saving
    ...
    try {
      ...
      if (response.ok) {
        ...
        // After saving, we might want to refresh the editableBlocks state from the (now potentially modified) pageDataJson
        // or simply update the state to reflect the saved blocks
